...
class Agent(object):
    ...
    def run2(self, state):
        """run suggests an action for the given state."""
        with torch.no_grad():
            return self.__policy(state).max(1).indices
    ...
    def learn(self, memory: ReplayMemory, batch_size: int) -> float:
        ...
        # double DQN
        v2 = self.__target(next_batch.float()).detach()
        values_next = torch.zeros([batch_size]).cuda()
        next_action_batch = self.run2(next_batch).view(32,1)
        values_next = v2.gather(1,next_action_batch).view(32)
        ...
        self.learnpoint+=torch.mean(values)
        if(self.eps_learn%10000==0):
            print(self.learnpoint/float(10000))
            avg_reward=self.learnpoint.item()/float(10000)
            self.learnpoint=0
            with open("dueling_avg_value.txt", "a") as fp:
                fp.write(f"{self.eps_learn//10000:3d} {avg_reward:f}\n")
        self.eps_learn+=1
